...
\begin{figure}[ht]
  \begin{tikzpicture}[node distance=2cm]
    ...
    \node (dataProcessing) [process, right of=dataCollection, xshift=2cm] {Data Processing};
    \node (modelLoding) [process, right of=dataProcessing, xshift=2cm] {Model Loding};
    \node (trainingModel) [process, below of=modelLoding] {Training Model};
    \node (evalModel) [process, left of=trainingModel, xshift=-2cm] {Evaluating Models};
    \node (packingModel) [process, left of=evalModel, xshift=-2cm] {Packing Selected Models};
    \node (vsgen) [process, below of=evalModel] {Vscore Generation};
    \node (selectModel) [process, left of=vsgen, xshift=-2cm] {Selection of Most suitable model};
    \node (end) [startstop, left of=packingModel, xshift=-1.5cm] {end};
    \node (modelTemplate) [file, above of=modelLoding] {Model Templates};
    \node (userPref) [file, right of=vsgen, xshift=2cm] {User Preferences};
    \draw [arrow] (start) -- (dataCollection);
    \draw [arrow] (dataCollection) -- (dataProcessing);
    \draw [arrow] (dataProcessing) -- (modelLoding);
    \draw [arrow] (modelTemplate) -- (modelLoding);
    \draw [arrow] (modelLoding) -- (trainingModel);
    \draw [arrow] (trainingModel) -- (evalModel);
    \draw [arrow] (evalModel) -- (vsgen);
    \draw [arrow] (userPref) -- (vsgen);
    \draw [arrow] (vsgen) -- (selectModel);
    \draw [arrow] (selectModel) -- (packingModel);
    \draw [arrow] (packingModel) -- (end);
  \end{tikzpicture}
  \caption{System Architecture - Selection Process}
  \label{flow:system_architecture_selection_process}
\end{figure}
